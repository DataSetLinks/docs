...
OntologyObject("Shops");
    ...
    .select("name, NAICS_code, distance") // Select relevant fields for output
    .get();
```
...
**Risk Level** (if added): A classification of the risk (e.g., High, Medium, Low).

| Name               | NAICS CODE | Distance (m) | Risk Level |
| ------------------ | ---------- | ------------ | ---------- |
