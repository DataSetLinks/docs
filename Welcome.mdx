...
Line by line:
6 - The inference can have multiple steps which are combined by our AI. The example above extracts a simple table, but you can have other steps suce has **table**, **rows**, **normalize**.
7 - In this specific example we specify we want a **table** extracted from a specific column. This will extract all columns available from the text provided. You can use the **helperPrompt** to guide the AI system to have higher accuracy during extraction.
