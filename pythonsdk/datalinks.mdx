...
* **Parameters:**
  ...
  * **model** (*str*) – The model name to use for inference.
  * **provider** (*str*) – The provider of the LLM model (ollama, openai, etc)
  * **include_reasoning** (*bool*) – Whether to include the reasoning steps in the response.
* **Returns:**
  A tuple with the agent response and the reasoning steps (dict) if include_reasoning is True, or just the response otherwise.
* **Return type:**
  Any
* **Raises:**
  **requests.exceptions.RequestException** – If a request-related error
...
### *class* datalinks.links.EntityResolutionTypes
...
### *class* datalinks.pipeline.StepTypes
...
### *class* datalinks.pipeline.NormalizeModes
...
### *class* datalinks.pipeline.ValidateModes
